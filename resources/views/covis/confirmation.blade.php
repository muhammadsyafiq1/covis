...
@section('content')
                    ...
                    <div class="nk-block-head nk-block-head-sm">
                        <div class="nk-block-between">
                            <div class="nk-block-head-content">
                                <h3 class="nk-block-title page-title mb-0">Confirmation Order / Request</h3>
                        ...
                        <div class="nk-block nk-block-lg">
                            <div class="card card-bordered card-preview">
                                <div class="card-inner">
                                    <form action="{{ url('confirmationAllTransaction') }}" method="post">
                                    @csrf
                                    <table class="datatable-init-export nk-tb-list nk-tb-ulist nowrap" data-auto-responsive="false"
                                        id="table_confirm" data-export-title="Export">
                                        <thead>
                                                    ...
                                                    <td class="nk-tb-col nk-tb-col-tools">
                                                        <ul class="nk-tb-actions gx-1">
                                                            <li class="nk-tb-action-hidden">
                                                                <a href="{{ route('transaction-confirmation-view',$tran->uuid) }}"
                                                            ...
                                                            @hasrole('Supervisor')
                                                                @if ($tran->status == 1)
                                                                    <li class="nk-tb-action-hidden">
                                                                        {{-- {{route('customer-waiting-approval',$tran->id)}} --}}
                                                                        <a href="" data-id="{{ $tran->id }}" data-name="{{ $tran->customer->name ?? 'N/A'}}"
                                                                            class="toggle btn btn-trigger btn-icon btn-confirm"
                                                                            data-target="confirm-modal" data-toggle="tooltip"
                                                                            data-placement="top" title="Confirm">
    ...
    <div class="nk-add-product toggle-slide toggle-slide-right" data-content="confirm-modal" data-toggle-screen="any"
        data-toggle-overlay="true" data-toggle-body="true" data-simplebar>
        <div class="nk-block-head">
            <div class="nk-block-head-content">
                <h5 class="nk-block-title">Confirmation Order / Request</h5>
                <div class="nk-block-des">
                    <p class="small text-primary">Are you sure want to confirm this order / request?
                        <span id="name"></span>
                    </p>
                </div>
            </div>
        ...
        <div class="nk-block">
            <form action="{{ route('transaction-waiting-approval') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-12">
                        <input type="hidden" name="covisTransaction_id" id="id" class="id">
                        {{-- <p id="coba"></p> --}}
                        <button class="btn btn-primary" type="submit"><span>Confirm</span></button>
                    </div>
                </div>
            </form>
        </div>
...
@push('addon-script')
    <script>
        $('#table_confirm').on('click','.btn-activated-deactivated', function(){
            let id = $(this).data('id');
    ...
    <script>
        $('#table_confirm').on('click', '.btn-confirm', function() {
            let id = $(this).data('id');
            let name = $(this).data('name');
            $('.id').val(id);
            $('#name').html(name);
    ...
    <script>
        $('#table_confirm').on('click', '.btn-approval', function() {
            let id = $(this).data('id');
            let name = $(this).data('name');
            $('.id').val(id);
            $('#name').html(name);
    ...
    <script>
        ...
        // function myFunction() {
        //     var x = document.getElementById("btn_confirm");
